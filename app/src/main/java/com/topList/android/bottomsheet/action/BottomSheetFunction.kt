...
import com.topList.android.utils.KeyboardUtils
import com.topList.theme.dp2px
import com.topList.theme.px2dp
...
class BottomSheetFunction(var sheet: ActionSheetLayout, private val hiddenAction: (()->Unit)? = null) : ActionFunction {
                    ...
                    override fun onVisibility(visible: Boolean) {
                        if (visible) {
                            KeyboardUtils.hideKeyboard(sheet, Runnable {
                                if (hasPeekState) {
                                    sheet.peek(sheet.height - peekHeight.dp2px.toInt())
                                }
                            }, 100)
                        }
                    }
            ...
            when {
                hasPeekState -> when {
                    velocityY > velocityYBound && velocityY > 0 -> when {
                        ...
                        else -> sheet.peek(totalYRange - peekHeight.dp2px.toInt())
                    }
                    velocityY < -velocityYBound && velocityY < 0 -> when {
                        ...
                        Math.abs(deltaY) > halfLimit -> sheet.peek(totalYRange - peekHeight.dp2px.toInt())
                        else -> sheet.expand()
                    }
                }
                else -> when {
    ...
    fun ActionSheetLayout.open(action: Runnable = Runnable {  }, duration: Int = 300, isNeedNotify: Boolean = true) {
        peekHeight = if (peekHeight > 0) peekHeight else sheet.height.px2dp.toInt()
        if(peekHeightF > 0) {
            peekHeight = (sheet.height * peekHeightF).px2dp.toInt()
        }
        if (hasPeekState) {
            peek(sheet.height - peekHeight.dp2px.toInt(), action, duration, isNeedNotify)
        } else {
